<x-app-layout>
    <x-slot name="header">
        <h2 class="font-bold leading-tight text-center">
            Onze Professionals
        </h2>
    </x-slot>
    ...
    <div id="professionals" class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
        @foreach($professionals as $prof)
            <div class=" w-18 rounded-xl shadow-lg overflow-hidden">
                <img src="{{ asset('storage/' . $prof->avatar) }}" alt="{{ $prof->first_name }} {{ $prof->last_name }}" class="w-full h-56 object-cover">
                <div class="p-2">
                    <h3 class="text-xl text-black font-semibold mb-2">{{ $prof->first_name }} {{ $prof->last_name }}</h3>
                    <p class="text-black text-opacity-80 mb-5"><i class="fas fa-graduation-cap"></i> {{ $prof->branch }}</p>
